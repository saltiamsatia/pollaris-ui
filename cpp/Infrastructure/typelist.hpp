...
/// This namespace contains infrastructure code which supports the application code, but is not aware of it
namespace infra {
...
/// Compare two typelists for exact equality of members and order
template<class... A, class... B>
constexpr bool operator==(list<A...>, list<B...>) { return std::is_same_v<list<A...>, list<B...>>; }
template<class... A, class... B>
constexpr bool operator!=(list<A...> a, list<B...> b) { return !(a == b); }
...
/// builder_t<>::add<T1>::add<T2>::add<T3>[...]::finalize
/// Or:
/// builder_t<>::add_list<list<T1, T2>>::add_list<T3, T4>>[...]::finalize
...
template<typename List = list<>>
using builder_t = typename builder<List>::type;
 ...
 */
template<typename... Types, typename Unsigned, typename Callable,
         typename = std::enable_if_t<impl::length<Types...>::value != 0 && std::is_unsigned_v<Unsigned>>,
         typename Return = decltype(std::declval<Callable>()(wrapper<at<list<Types...>, 0>>()))>
Return dispatch(list<Types...>, Unsigned index, Callable c) {
...
template<typename List, typename Signed, typename Callable, typename = std::enable_if_t<std::is_signed_v<Signed>>>
auto dispatch(List l, Signed index, Callable c) {
...
/// @brief Invoke the provided predicate with an argument wrapper<Type>() and return true iff every type matches
template<typename... Types, typename Predicate>
bool all_of(list<Types...>, Predicate p) {
    using List = list<Types...>;
    if constexpr (length<List>() == 0) return true;
    else {
        static_assert(std::is_same_v<decltype(p(wrapper<first<List>>())), bool>, "Predicate must return a bool");
        if (p(wrapper<first<List>>())) return all_of(slice<List, 1>(), p);
        return false;
    }
}

/// @brief Invoke the provided predicate with an argument wrapper<Type>() and return true iff any type matches
template<typename... Types, typename Predicate>
bool any_of(list<Types...>, Predicate p) {
    using List = list<Types...>;
    if constexpr (length<List>() == 0) return false;
    else {
        static_assert(std::is_same_v<decltype(p(wrapper<first<List>>())), bool>, "Predicate must return a bool");
        if (p(wrapper<first<List>>())) return true;
        return any_of(slice<List, 1>(), p);
    }
}

/// @brief Invoke the provided predicate with an argument wrapper<Type>() and return true iff no type matches
template<typename... Types, typename Predicate>
bool none_of(list<Types...>, Predicate p) {
    using List = list<Types...>;
    if constexpr (length<List>() == 0) return true;
    else {
        static_assert(std::is_same_v<decltype(p(wrapper<first<List>>())), bool>, "Predicate must return a bool");
        if (p(wrapper<first<List>>())) return false;
        return none_of(slice<List, 1>(), p);
    }
}
...
/// @brief Convert a typelist to a tuple by instantiating each type with all of the provided arguments
template<typename... Args, typename... Types>
std::tuple<Types...> make_tuple(list<Types...>, Args... args) {
    return std::make_tuple(Types(std::forward<Args>(args)...)...);
}
